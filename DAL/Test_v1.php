...
* Date Generated : 03.06.2019 13h
* File name : Test.php
* Table : meryeme_comptabad.Test 
* -----------------------------------------------------------------------------------
*/
...
class Test extends Common
{
    ...
    private $structure = array(
        'ID_Test' => array('ID_Test', 'int', '2', '0', ''),
        'Nom' => array('Nom', 'varchar-50', '0', '0', ''),
        'DateModif' => array('DateModif', 'datetime', '0', '0', '')
    );
    ...
    function __construct ($val = null, $property = self::primary_key)
    {
        ...
        {
            $query = $Connexion->sql_query("SELECT * FROM `Test` WHERE `$property` = ".parent::quote($this->structure[$property],$val) ." LIMIT 1");

            if($Connexion->sql_num_rows($query) != 0)
            {
                while($row = $Connexion->sql_fetch_object($query))
                {
                    $this->ID_Test = $this->formater($this->structure['ID_Test'],$row->ID_Test);
                    $this->structure['ID_Test'][4] = $this->ID_Test;
                    $this->Nom = $this->formater($this->structure['Nom'],$row->Nom);
                    $this->structure['Nom'][4] = $this->Nom;
                    $this->DateModif = $this->formater($this->structure['DateModif'],$row->DateModif);
                    $this->structure['DateModif'][4] = $this->DateModif;
    ...
    public function get_ID_Test()
    {
        return stripslashes($this->ID_Test);
    }
    ...
    public function get_DateModif()
    {
        return stripslashes($this->DateModif);
    }

    public function set_DateModif($valeur = null)
    {
        if ( ( $this->DateModif != parent::quote($this->structure['DateModif'][1],$valeur) ) && ( $this->DateModif != $valeur ) )
        {	
            if(is_null($valeur))
            {
                $this->DateModif = NULL;
            ...
            {
                $this->DateModif = parent::quote($this->structure['DateModif'][1],$valeur);
            }
            $this->isToSaveOrToUpdate=1;
            $this->structure['DateModif']["3"]=1;
            $this->structure['DateModif']["4"] = $this->DateModif;

            $test = explode("-",$this->structure['DateModif'][1]);
            if ( ( $test["0"] == "varchar" ) && ( isset( $test["1"] ) ) )
            {
                $this->structure['DateModif']["4"] = substr($this->structure['DateModif']["4"],0,$test["1"]);
            }
    ...
    public function delete($transaction = null)
    {
        ...
        if ((isset($this->ID_Test))&&($this->ID_Test!=0))
        {
	...
	if (($transaction=="On"))
            {
                $query = parent::makequery('DELETE', $Connexion->Database, 'Test', $this->structure);
    ...
    {		
        ...
        if ((is_null($this->ID_Test))||($this->ID_Test==0))
        {
            $this->isToSaveOrToUpdate=1;
            $this->isNew=1;
        }
        ...
        if ($this->isToSaveOrToUpdate==1)
        {
            ...
            if ((isset($this->ID_Test))&&($this->ID_Test!="0")&&($this->isNew!=1))
            {
                $query = parent::makequery('UPDATE', $Connexion->Database, 'Test', $this->structure);
                $result=$Connexion->sql_query($query);
            }
            else
            {
                $query = parent::makequery('INSERT', $Connexion->Database, 'Test', $this->structure);
                $Connexion->sql_query($query);
                $this->ID_Test=$Connexion->sql_insert_id();
                $this->structure['ID_Test']["4"] = $this->ID_Test;
                $this->isNew=0;
            }
        }
    ...
    public function last_insert($property = self::primary_key)
    {
        ...
        $query = $Connexion->sql_query("SELECT $property AS last FROM `Test` ORDER BY ".self::primary_key." DESC LIMIT 1");
