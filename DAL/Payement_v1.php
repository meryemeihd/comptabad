...
class Payement extends Common
{
    ...
    private $structure = array(
        ...
        'Reçu' => array('Reçu', 'varchar-100', '1', '0', ''),
        ...
        if((isset($val))&&(trim($val)!=""))
        {
            ...
            if($Connexion->sql_num_rows($query) != 0)
            {
                while($row = $Connexion->sql_fetch_object($query))
                {
                    ...
                    $this->Reçu = $this->formater($this->structure['Reçu'],$row->Reçu);
                    $this->structure['Reçu'][4] = $this->Reçu;
    ...
    public function get_Reçu()
    {
        return stripslashes($this->Reçu);
    }

    public function set_Reçu($valeur = null)
    {
        if ( ( $this->Reçu != parent::quote($this->structure['Reçu'][1],$valeur) ) && ( $this->Reçu != $valeur ) )
        {	
            if(is_null($valeur))
            {
                $this->Reçu = NULL;
            ...
            {
                $this->Reçu = parent::quote($this->structure['Reçu'][1],$valeur);
            }
            $this->isToSaveOrToUpdate=1;
            $this->structure['Reçu']["3"]=1;
            $this->structure['Reçu']["4"] = $this->Reçu;

            $test = explode("-",$this->structure['Reçu'][1]);
            if ( ( $test["0"] == "varchar" ) && ( isset( $test["1"] ) ) )
            {
                $this->structure['Reçu']["4"] = substr($this->structure['Reçu']["4"],0,$test["1"]);
            }
