...
class Facture extends Common
{
    ...
    private $structure = array(
        ...
        'Donneur' => array('Donneur', 'mediumtext', '1', '0', ''),
        ...
        if((isset($val))&&(trim($val)!=""))
        {
            ...
            if($Connexion->sql_num_rows($query) != 0)
            {
                while($row = $Connexion->sql_fetch_object($query))
                {
                    ...
                    $this->Donneur = $this->formater($this->structure['Donneur'],$row->Donneur);
                    $this->structure['Donneur'][4] = $this->Donneur;
    ...
    public function get_Donneur()
    {
        return stripslashes($this->Donneur);
    }

    public function set_Donneur($valeur = null)
    {
        if ( ( $this->Donneur != parent::quote($this->structure['Donneur'][1],$valeur) ) && ( $this->Donneur != $valeur ) )
        {	
            if(is_null($valeur))
            {
                $this->Donneur = NULL;
            ...
            {
                $this->Donneur = parent::quote($this->structure['Donneur'][1],$valeur);
            }
            $this->isToSaveOrToUpdate=1;
            $this->structure['Donneur']["3"]=1;
            $this->structure['Donneur']["4"] = $this->Donneur;

            $test = explode("-",$this->structure['Donneur'][1]);
            if ( ( $test["0"] == "varchar" ) && ( isset( $test["1"] ) ) )
            {
                $this->structure['Donneur']["4"] = substr($this->structure['Donneur']["4"],0,$test["1"]);
            }
